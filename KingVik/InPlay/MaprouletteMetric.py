...
class MapRouletteStats:
    ...
    def save_csv(self, data, out_file):
        """Save data to CSV, completely replacing old file"""
        fields = [
        ...
        # Write to CSV (completely fresh file)
        with open(out_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=fields)
            writer.writeheader()
            writer.writerows(data)

        print(f"✅ Saved fresh data at {out_file} (Total records: {len(data)})")


def main():
    challenges = [52701, 52700, 52147, 52698,52708,52699,52706,52707,52709,52704,52702,
                  52560,52579,52716,



                  ]  # All challenge IDs to include
    # out_folder = "C:\\Users\\chukw\\OneDrive\\Documentos\\TomTom Event"
    out_folder = "G:\My Drive\Dashboard\TomTom Power BI\MapRoulette"
    out_file = os.path.join(out_folder, "maproulette_metrics.csv")
